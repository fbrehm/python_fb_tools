...
class LockHandler(BaseHandler):
    ...
    def _do_create_lockfile(
        ...
        try:
            ...
            msg = _("Got a lock for lockfile {!r}.").format(lockfile)
            if self.silent:
                LOG.debug(msg)
            else:
                LOG.info(msg)
    ...
    def check_lockfile(self, lockfile, max_age=None, use_pid=None):
        ...
        if use_pid:
            ...
            if pid is None:
                LOG.warn(_("Unusable lockfile {!r}.").format(lockfile))
            else:
                if self.dead(pid):
    ...
    def get_pid_from_file(self, pidfile, force=False):
        ...
        if pid <= 0:
            msg = _("Invalid PID {pid} in {file!r} found.").format(pid=pid, file=pidfile)
            if force:
                LOG.warn(msg)
                return None
            else:
