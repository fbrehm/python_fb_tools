...
class TestFbMultiConfig(FbToolsTestcase):
    ...
    def test_import(self):
        ...
        LOG.debug(
            "Version of fb_tools.multi_config: {!r}.".format(fb_tools.multi_config.__version__)
        )
    ...
    def test_init_stems(self):
        ...
        for stem in invalid_stems:
            LOG.debug(
                "Testing invalid stem {s!r} ({c}).".format(s=stem, c=stem.__class__.__name__)
            )
    ...
    def test_collect_cfg_files(self):
        ...
        for ext in exts:
            ...
            if path.exists():
                ...
                LOG.debug(
                    "Checking method {m!r} of cfg file {f!r}.".format(m=exp_method, f=str(path))
                )
    ...
    def test_read_charset(self):
        ...
        test_stems = (
            "test_multicfg-latin1",
            "test_multicfg-utf-16",
            "test_multicfg-utf-32",
            "test_multicfg-utf8",
        )
    ...
    def test_read_broken(self):
        ...
        for test_token in test_stem_tokens:
            ...
            if not do_test:
                LOG.info(
                    (
                        "Testing for file stem {!r} not executed, appropriate module " "not there."
                    ).format(stem)
                )
                continue
            LOG.info("Testing for file stem {!r} ...".format(stem))
    ...
    def test_evaluation(self):
        ...
        cfg = BaseMultiConfig(
            ...
            config_dir=self.test_cfg_dir.name,
            additional_cfgdirs=self.test_cfg_dir,
            verbose=used_verbose,
            append_appname_to_stems=False,
            additional_stems=test_stem,
        )
        ...
        with self.assertRaises(RuntimeError) as cm:
            cfg.eval()
        e = cm.exception
        LOG.info("{c} raised on unread configuration: {e}".format(c=e.__class__.__name__, e=e))
    ...
    def test_additional_config_file(self):
        ...
        for test_add_config in wrong_configs:
            ...
            e = cm.exception
            LOG.debug(
                "{c} raised on not usable config file {fn!r}: {e}".format(
    ...
    def test_checking_privacy(self):
        ...
        LOG.debug(
            "Using modes private '{priv:04o}' and public '{pub:04o}'.".format(
        ...
        with tempfile.NamedTemporaryFile(
            mode="w+b",
            buffering=0,
            prefix="test_multicfg-",
            suffix=".ini",
            dir=str(self.test_cfg_dir),
        ) as fh:
            ...
            e = cm.exception
            LOG.info(
                "{c} raised on public visible config file: {e}".format(c=e.__class__.__name__, e=e)
            )
