...
class TestFbHandlingObject(FbToolsTestcase):
    ...
    def test_import(self):
        ...
        LOG.debug(
            "Version of fb_tools.handling_obj: {!r}.".format(fb_tools.handling_obj.__version__)
        )
        ...
        LOG.debug(
            "Description of CalledProcessError: " + textwrap.dedent(CalledProcessError.__doc__)
        )
        ...
        LOG.debug(
            "Description of TimeoutExpiredError: " + textwrap.dedent(TimeoutExpiredError.__doc__)
        )

        LOG.info("Testing import of HandlingObject from fb_tools.handling_obj ...")
        from fb_tools.handling_obj import HandlingObject

        LOG.debug("Description of HandlingObject: " + textwrap.dedent(HandlingObject.__doc__))

        LOG.info("Testing import of CompletedProcess from fb_tools.handling_obj ...")
        from fb_tools.handling_obj import CompletedProcess

        LOG.debug("Description of CompletedProcess: " + textwrap.dedent(CompletedProcess.__doc__))
    ...
    def test_run_timeout(self):
        ...
        LOG.debug(
            "Trying to execute {c!r} with a timeout of {t} seconds ...".format(
    ...
    def test_get_command(self):
        ...
        LOG.debug(
            (
                "Searching symlinked command {!r}, which points to {!r} " "with resolved path."
            ).format(symlink, cmd)
        )
    ...
    # -------------------------------------------------------------------------
    @unittest.skipUnless(EXEC_CONSOLE_TESTS, "Tests depending on a console are not executed.")
    def test_get_password(self):
        ...
        while i < leaps:
            ...
            try:
                ...
                LOG.info(
                    "Got a {c}: Interrupted on demand - {e}.".format(c=e.__class__.__name__, e=e)
                )
            else:
