...
class TestSocketHandler(FbToolsTestcase):
    ...
    def test_import(self):
        ...
        LOG.debug("Version of fb_tools.socket_obj: {!r}".format(fb_tools.socket_obj.__version__))
    ...
    def test_base_object(self):
        ...
        e = cm.exception
        LOG.debug(
            "{cls} raised on instantiate a GenericSocket: {err}".format(
    ...
    def test_minimal_object(self):
        ...
        for to in ("bla", -10, MAX_TIMEOUT + 1000):
            ...
            e = cm.exception
            LOG.debug(
                "{cls} raised on setting timeout {to!r}: {err}".format(
        ...
        for rqs in ("bla", -5, MAX_REQUEST_QUEUE_SIZE * 3):
            ...
            e = cm.exception
            LOG.debug(
                "{cls} raised on setting request_queue_size {to!r}: {err}".format(
        ...
        for bs in ("blub", -512, 0, MIN_BUFFER_SIZE - 10, MAX_BUFFER_SIZE * 5, 5120 + 10):
            ...
            e = cm.exception
            LOG.debug(
                "{cls} raised on setting buffer_size {to!r}: {err}".format(
        ...
        for enc in (0, "bla", FbBaseObject()):
            ...
            e = cm.exception
            LOG.debug(
                "{cls} raised on setting encoding {enc!r}: {err}".format(
