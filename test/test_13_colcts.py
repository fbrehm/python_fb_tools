...
class TestFbCollections(FbToolsTestcase):
    ...
    def test_init_frozenset(self):
        ...
        LOG.debug("Checking as_dict(), source: {src}, expeced: {ex}.".format(src=src, ex=expected))
        ...
        e = cm.exception
        msg = (
            "AttributeError raised on trying to add a value to a " "FrozenCIStringSet object: {}"
        ).format(e)
        LOG.debug(msg)
        ...
        for test_tuple in correct_iterables:
            ...
            LOG.debug(
                "FrozenCIStringSet as a list: {r!r} (expeced: {ex!r})".format(
        ...
        wrong_iterables = ("a", 1, {"uhu": "banane"}, tobj, tobj.uhu)
    ...
    def test_frozenset_real_value(self):
        ...
        e = cm.exception
        msg = (
            "WrongItemTypeError raised on real_value() of a " "FrozenCIStringSet object: {}"
        ).format(e)
        LOG.debug(msg)
        ...
        with self.assertRaises(KeyError) as cm:
            ...
            LOG.debug("Got a value {!r}.".format(value))
        e = cm.exception
        msg = ("KeyError raised on real_value() of a FrozenCIStringSet object: {}").format(e)
        LOG.debug(msg)
    ...
    def test_frozenset_operator_in(self):
        ...
        for item in valid_items:
            LOG.debug(
                "Testing, that {i!r} is a member of {s!r}.".format(i=item, s=my_set.as_list())
            )
            self.assertIn(item, my_set)

        for item in invalid_items:
            LOG.debug(
                "Testing, that {i!r} is NOT a member of {s!r}.".format(i=item, s=my_set.as_list())
            )
            self.assertNotIn(item, my_set)

        for item in wrong_items:
            LOG.debug(
                "Testing, that {i!r} has the wrong type to be a member of {s!r}.".format(
    ...
    def test_frozenset_operator_xor(self):
        ...
        e = cm.exception
        msg = (
            "WrongCompareSetClassError on a symmetric " "difference with a wrong object: {}"
        ).format(e)
        LOG.debug(msg)
    ...
    def test_init_set(self):
        ...
        for test_tuple in correct_iterables:
            ...
            result = my_set.as_list()
            LOG.debug(
                "CIStringSet as a list: {r!r} (expeced: {ex!r})".format(r=result, ex=expected)
            )
            self.assertEqual(result, expected)
    ...
    def test_init_frozendict(self):
        ...
        LOG.debug(
            "Checking as_dict(), source: {src}, expeced: {ex}.".format(
        ...
        for test_tuple in test_tuples:
            ...
            if self.verbose > 2:
                LOG.debug(
                    "FrozenCIDict as a dict: {r} (expeced: {e})".format(
    ...
    def test_frozendict_real_key(self):
        ...
        for test_tuple in test_tuples:
            ...
            if self.verbose > 2:
                LOG.debug(
                    "Testing to get the real key of {v!r} of {s}.".format(v=key, s=my_dict.dict())
                )
    ...
    def test_frozendict_get(self):
        ...
        for test_tuple in test_tuples:
            ...
            if self.verbose > 2:
                LOG.debug(
                    "Testing to get the value of key {v!r} of {s} with my_dict[key].".format(
            ...
            if self.verbose > 2:
                LOG.debug(
                    (
                        "Testing to get the value of key {v!r} of {s} with " "my_dict.get(key)."
                    ).format(v=key, s=my_dict.dict())
                )
    ...
    def test_frozendict_contains(self):
        ...
        for key in wrong_keys:
            ...
            if self.verbose > 2:
                msg = (
                    '{n} raised on key "{k!r} in src_dict" of a FrozenCIDict ' "object: {e}"
                ).format(n=e.__class__.__name__, k=key, e=e)
                LOG.debug(msg)
    ...
    def test_frozendict_operator_eq(self):
        ...
        for test_tuple in test_tuples:
            ...
            if should_be_equal:
                ...
                if self.verbose > 2:
                    LOG.debug(
                        "Testing, that {!r} is NOT equal to src_dict ...".format(comp_object)
                    )
    ...
    def test_init_dict(self):
        ...
        for test_tuple in test_tuples:
            ...
            if self.verbose > 2:
                LOG.debug(
                    "CIDict as a dict: {r} (expeced: {e})".format(r=pp(result), e=pp(expected))
                )
            self.assertEqual(result, expected)
    ...
    def test_dict_set(self):
        ...
        for test_tuple in test_tuples:
            ...
            if self.verbose > 2:
                LOG.debug(
                    (
                        ...
                        "dict {exd}."
                    ).format(v=result_val, d=result_dict, exv=value, exd=expected)
                )
            ...
            if self.verbose > 2:
                LOG.debug(
                    ...
                    ).format(k=key, v=value, s=my_dict.dict())
                )
        ...
        for key in wrong_keys:
            ...
            if self.verbose > 2:
                msg = (
                    "Trying to set key {k!r} to value {v!r} in {s} with " "src_dict[key] ..."
                ).format(k=key, v=value, s=src)
                LOG.debug(msg)
            with self.assertRaises(FbCollectionsError) as cm:
                src_dict[key] = value
            e = cm.exception
