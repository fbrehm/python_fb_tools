...
class TestMailaddress(FbToolsTestcase):
    ...
    def test_import(self):
        ...
        LOG.debug("Version of fb_tools.mailaddress: {!r}".format(fb_tools.mailaddress.__version__))
    ...
    def test_empty_address(self):
        ...
        LOG.debug("Empty MailAddress {a!r}: {s!r}".format(a=address, s=str(address)))
    ...
    def test_wrong_addresses(self):
        ...
        for token in correct_addresses:
            ...
            LOG.debug(
                "Successful mail address from {s!r}: {a!r} => {r!r}".format(
    ...
    def test_wrong_user(self):
        ...
        for user in correct_users:
            ...
            LOG.debug(
                "Successful mail address from {u!r} (@{d}): {a!r} => {r!r}".format(
    ...
    def test_to_str(self):
        ...
        for row in data:
            address = MailAddress(user=row[0], domain=row[1], verbose=self.verbose, empty_ok=True)
            LOG.debug("Testing typecasting or address {!r}.".format(address))
            LOG.debug(
                "Expected: Str(adress): {s!r}, adress.str_for_access(): {a!r}".format(
                    s=row[2], a=row[3]
                )
            )
            ...
            LOG.debug(
                "Str(adress): {s!r}, adress.str_for_access(): {a!r}".format(
    ...
    def test_empty_qualified_address(self):
        ...
        LOG.debug("Empty QualifiedMailAddress {a!r}: {s!r}".format(a=address, s=str(address)))
    ...
    def test_wrong_init_full_address(self):
        ...
        LOG.debug("Testing raise on init qualified mail address with wrong keyword arguments ...")
    ...
    def test_wrong_qual_address(self):
        ...
        for token in correct_addresses:
            ...
            expected = token[1]
            LOG.debug(
                "Testing qualified mail address {a!r} => {e!r} ...".format(a=addr, e=expected)
            )
            address = QualifiedMailAddress(addr, verbose=self.verbose)
            LOG.debug(
                "Successful qualified mail address from {s!r}: {a!r} => {r!r}".format(
                    s=addr, a=str(address), r=address
                )
            )
            self.assertEqual(str(address), expected)
        ...
        wrong_addresses = (
            ...
            "uhu_banane.de",
            "@uhu-banane.de",
            '"Frank Brehm <frank.uwe@banane.de>',
            'Frank Brehm" <frank.uwe@banane.de>',
            '<frank.uwe@banane.de> "Frank Brehm"',
            "Brehm < Frank <frank.uwe@banane.de>",
            "Brehm < Frank <frank.uwe@banane.de>",
            "Brehm@Frank <frank.uwe@banane.de>",
            "Brehm|Frank <frank.uwe@banane.de>",
        )
