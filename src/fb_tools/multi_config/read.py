...
class MultiCfgReadMixin:
    ...
    def detect_file_encoding(self, cfg_file, force=False):
        ...
        if self.verbose > 2:
            msg = _(
                "Found character set {cs!r} for file {fn!r} with a confidence of " "{con:0.1f}%."
            ).format(cs=encoding, fn=str(cfg_file), con=(confidence * 100))
            LOG.debug(msg)
    ...
    def load_hjson(self, cfg_file):
        ...
        LOG.debug(
            _("Reading {tp} file {fn!r} ...").format(tp="human readable JSON", fn=str(cfg_file))
        )
