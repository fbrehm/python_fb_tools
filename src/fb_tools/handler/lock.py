...
class LockdirNotWriteableError(LockHandlerError):
    ...
    def __str__(self):
        """Typecaste into a string for error output."""
        return _("Locking directory {!r} isn't writeable.").format(str(self.lockdir))
        ...
        **kwargs,
        ...
        if not lfile.exists():
            if self.simulate:
                LOG.info(
                    _(
                        "Lockfile {!r} doesn't exists, but don't worry, " "it's simulation mode."
                    ).format(str(lockfile))
                )
            else:
            ...
            if not lfile.is_file():
                raise LockObjectError(
                    _("Lockfile {!r} is not a regular file.").format(str(lockfile))
                )
...
class LockHandler(BaseHandler):
    ...
    def check_for_number(self, value, default, what, must_gt_zero=False, must_ge_zero=False):
        ...
        if not isinstance(value, Number):
            msg = _("Value {val!r} for {what} is not a Number.").format(val=value, what=what)
            raise LockHandlerError(msg)
        ...
        if must_ge_zero and value < 0:
            msg = _(
                "The value for {what} must be greater than " "or equal to zero (is {val!r})."
            ).format(val=value, what=what)
            raise LockHandlerError(msg)
    ...
    def create_lockfile(
        ...
        delay_start = self.check_for_number(
            delay_start, self.lockretry_delay_start, what="delay_start", must_gt_zero=True
        )

        delay_increase = self.check_for_number(
            delay_increase, self.lockretry_delay_increase, what="delay_increase", must_ge_zero=True
        )

        max_delay = self.check_for_number(
            max_delay, self.lockretry_max_delay, what="max_delay", must_ge_zero=True
        )
        ...
        max_age = self.check_for_number(
            max_age, self.max_lockfile_age, what="max_age", must_ge_zero=True
        )
    ...
    def _do_create_lockfile(
        ...
        try:
            ...
            while fd is None and time_diff < max_delay:
                ...
                LOG.debug(
                    _("Try {try_nr} on creating lockfile {lfile!r} ...").format(
            ...
            out = to_utf8("{}\n".format(pid))
            LOG.debug(
                _("Write {what!r} in lockfile {lfile!r} ...").format(what=out, lfile=str(lockfile))
            )
    ...
    def remove_lockfile(self, lockfile):
        ...
        try:
            ...
            LOG.error(
                _("Error on removing lockfile {lfile!r}: {err}").format(lfile=str(lockfile), err=e)
            )
            if self.verbose:
    ...
    def check_lockfile(self, lockfile, max_age=None, use_pid=None):
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                LOG.info(
                    _("Could not stat for file {lfile!r}: {err}").format(
        ...
        if age >= max_age:
            LOG.debug(
                _("Lockfile {lfile!r} is older than {max} seconds ({age} seconds).").format(
                    lfile=str(lfile), max=max_age, age=age
                )
            )
            return False
        msg = _(
            "Lockfile {lfile!r} is {age} seconds old, but not old enough " "({max} seconds)."
        ).format(lfile=str(lfile), max=max_age, age=age)
        LOG.debug(msg)
        return True
