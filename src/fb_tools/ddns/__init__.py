...
class BaseDdnsApplication(FbConfigApplication):
    ...
    def init_arg_parser(self):
        ...
        if proto_help is None:
            proto_help = _(
                ...
                "(one of {c}, default {d!r})."
            ).format(c=format_list(valid_list, do_repr=True, style="or"), d="any")
        ...
        ddns_group.add_argument(
            ...
            must_exists=self.work_directory_must_exists,
            writeable=self.work_directory_must_be_writeable,
            help=_(
                "The directory, where to read and write the cache files of the "
                "evaluated IP addresses (default: {!r})."
            ).format(str(DdnsConfiguration.default_working_dir)),
        )
    ...
    # -------------------------------------------------------------------------
    def perform_request(
        self, url, method="GET", data=None, headers=None, may_simulate=False, return_json=True
    ):
    ...
    def _mangle_response(self, response, return_json=True):
        ...
        if self.verbose > 3:
            ...
            LOG.debug(
                "Status of request: {c!r} - {r!r}".format(
    ...
    def verify_working_dir(self):
        ...
        if self.verbose > 1:
            LOG.debug(
                _("Checking existence and accessibility of working directory {!r} ...").format(
        ...
        if not os.access(str(self.cfg.working_dir), os.R_OK):
            raise WorkDirAccessError(self.cfg.working_dir, _("No read access"))

        if not os.access(str(self.cfg.working_dir), os.W_OK):
            raise WorkDirAccessError(self.cfg.working_dir, _("No write access"))
    ...
    def write_ip_cache(self, address, cache_file):
        """Write a cache entry."""
        LOG.debug(
            _("Writing IP address {a!r} into {f!r} ...").format(a=str(address), f=str(cache_file))
        )
        cont = str(address) + "\n"
        try:
        ...
        if not has_errors:
            ...
            for line in lines:
                ...
                try:
                    ...
                    msg = (
                        _("Line {li!r} in {f!r} is not a valid IP address:").format(
