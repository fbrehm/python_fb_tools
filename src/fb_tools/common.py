...
RE_FQDN = re.compile(
    r"(?=^.{4,253}$)(^((?!-)[a-z0-9-]{1,63}(?<!-)\.)+[a-z]{2,63}\.?$)", re.IGNORECASE
)
...
def human2mbytes(value, si_conform=False, as_float=False):
    ...
    if mbytes != int(mbytes):
        raise ValueError(
            "int {integer!r} != long {long!r}.".format(integer=int(mbytes), long=mbytes)
        )
...
# =============================================================================
def bytes2human(value, si_conform=False, precision=None, format_str="{value} {unit}"):
    """
