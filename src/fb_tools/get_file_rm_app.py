...
class KeepOptionAction(argparse.Action):
    ...
    def __init__(self, option_strings, min_val, *args, **kwargs):
        ...
        super(KeepOptionAction, self).__init__(*args, option_strings=option_strings, **kwargs)

    # -------------------------------------------------------------------------
    def __call__(self, parser, namespace, values, option_string=None):
        """Call method on parsing the option."""
        if values < self._min:
            msg = _("Value must be at least {m} - {v} was given.").format(m=self._min, v=values)
...
class GetFileRmApplication(BaseApplication):
    ...
    def init_arg_parser(self):
        ...
        keep_group.add_argument(
            ...
            dest="last",
            type=int,
            action=KeepOptionAction,
            min_val=self.min_keep_last,
            help=_(
                "How many of the last files should be kept "
                "(default: {default}, minimum: {min})?"
            ).format(default=self.default_keep_last, min=self.min_keep_last),
        )
        ...
        keep_group.add_argument(
            ...
            type=int,
            action=KeepOptionAction,
            min_val=self.min_keep_days,
            help=_(
                "How many files one per day from today on should be kept "
                "(default: {default}, minimum: {min})?"
            ).format(default=self.default_keep_days, min=self.min_keep_days),
        )
        ...
        keep_group.add_argument(
            ...
            type=int,
            action=KeepOptionAction,
            min_val=self.min_keep_weeks,
            help=_(
                "How many files one per week from today on should be kept "
                "(default: {default}, minimum: {min})?"
            ).format(default=self.default_keep_weeks, min=self.min_keep_weeks),
        )
        ...
        keep_group.add_argument(
            ...
            type=int,
            action=KeepOptionAction,
            min_val=self.min_keep_months,
            help=_(
                "How many files one per month from today on should be kept "
                "(default: {default}, minimum: {min})?"
            ).format(default=self.default_keep_months, min=self.min_keep_months),
        )
        ...
        keep_group.add_argument(
            ...
            type=int,
            action=KeepOptionAction,
            min_val=self.min_keep_years,
            help=_(
                "How many files one per year from today on should be kept "
                "(default: {default}, minimum: {min})?"
            ).format(default=self.default_keep_years, min=self.min_keep_years),
        )
    ...
    def _xlate_date_pattern(self):
        ...
        self._pattern = (
            ...
            .replace("%m", r"(?P<month>\d\d?)")
            .replace("%d", r"(?P<day>\d\d?)")
        )
    ...
    def get_files_to_keep_month(self, files_assigned):
        ...
        for month_str in files_assigned.keys():
            ...
            this_month = datetime.date(int(match.group(1)), int(match.group(2)), 1)
    ...
    def get_files_to_keep_week(self, files_assigned):
        ...
        for day_str in files_assigned.keys():
            ...
            this_day = datetime.date(int(match.group(1)), int(match.group(2)), int(match.group(3)))
